...
mod tests {
    use canvas_contracts::{
        ...
        nodes::NodeRegistry,
        types::{Connection, ExecutionContext, Port, Position, ValueType, VisualGraph, VisualNode},
    };
    use uuid::Uuid;
    ...
    fn test_execute_simple_arithmetic() {
        ...
        graph.add_node(
            VisualNode::new(add_id, "Add", Position::new(100.0, 0.0))
                .with_inputs(vec![
        ...
        graph.add_node(
            VisualNode::new(end_id, "End", Position::new(200.0, 0.0)).with_inputs(vec![Port::new(
                "flow_in",
                "Flow In",
                ValueType::Flow,
            )
        ...
        let add_step = trace
            .steps
            .iter()
            .find(|s| s.node_id == add_id)
            .expect("Add node should have executed");
    ...
    fn test_execute_branching_true() {
        ...
        graph.add_node(
            VisualNode::new(if_id, "If", Position::new(100.0, 0.0))
                .with_inputs(vec![
        ...
        graph.add_node(
            VisualNode::new(write_true_id, "WriteStorage", Position::new(200.0, -50.0))
                .with_inputs(vec![
                    Port::new("flow_in", "Flow In", ValueType::Flow).required(),
        ...
        graph.add_node(
            VisualNode::new(write_false_id, "WriteStorage", Position::new(200.0, 50.0))
                .with_inputs(vec![
                    Port::new("flow_in", "Flow In", ValueType::Flow).required(),
                    Port::new("key", "Key", ValueType::String).required(),
                    Port::new("value", "Value", ValueType::Any).required(),
        ...
        graph.add_node(
            VisualNode::new(end_id, "End", Position::new(300.0, 0.0)).with_inputs(vec![Port::new(
                "flow_in",
                "Flow In",
                ValueType::Flow,
            )
        ...
        // Verify storage
        assert_eq!(
            final_ctx.storage.get("path").unwrap(),
            &serde_json::json!("true_branch")
        );
    }
