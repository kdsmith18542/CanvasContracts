#[cfg(test)]
mod manifest_tests {
    use canvas_contracts::{compiler::Compiler, config::Config, types::VisualGraph};
    use std::fs;
    ...
    fn test_lockfile_and_validation_report_generation() {
        ...
        let lock_output_path = temp_dir.path().join("test_contract.lock.json");
        ...
        assert_eq!(parsed_lock["schema_version"], "canvas.graph.v1");
